...
function showHeist(ZinitialText, Ztable, Zmoney, Zxp)
    CreateThread(function()
        function drawHeist(_initialText, _table, _money, _xp)
            ...
            for key, scaleform_handle in pairs(scaleform_list) do
                ...
                if _table[1] ~= nil then
                    Scaleform.CallFunction(scaleform_handle, false, "CREATE_STAT_TABLE", 1, 20)
    
                    for i, k in pairs(_table) do
						if i <=7 then
							Scaleform.CallFunction(scaleform_handle, false, "ADD_STAT_TO_TABLE", 1, 20, _table[i].stat, _table[i].value, true, true, false, false, 0)
						end
                    end
    
                    Scaleform.CallFunction(scaleform_handle, false, "ADD_STAT_TABLE_TO_WALL", 1, 20)
                end
    
                if _money.startMoney ~= _money.finishMoney then
                    Scaleform.CallFunction(scaleform_handle, false, "CREATE_INCREMENTAL_CASH_ANIMATION", 1, 30)
                    Scaleform.CallFunction(scaleform_handle, false, "ADD_INCREMENTAL_CASH_WON_STEP", 1, 30, _money.startMoney, _money.finishMoney, _money.topText, _money.bottomText, _money.rightHandStat, _money.rightHandStatIcon, 3)
                    Scaleform.CallFunction(scaleform_handle, false, "ADD_INCREMENTAL_CASH_ANIMATION_TO_WALL", 1, 30)
                end
		...
		Scaleform.CallFunction(scaleform, false, "CLEANUP", 1)
		Scaleform:Dispose(scaleform)
		Scaleform:Dispose(scale_bg)
		Scaleform:Dispose(scale_fg)
